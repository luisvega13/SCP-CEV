...
import type { AlcanceBeca, Beca, TipoDescuentoBeca } from "@/types/database";
...
export default function ScholarshipConfigurationPage() {
  ...
  const [discountType, setDiscountType] = useState<TipoDescuentoBeca>("porcentaje");
  ...
  function openEditor(nextEditor: Exclude<EditorState, null>) {
    ...
    const nextType = nextEditor.scholarship?.tipo_descuento ?? "porcentaje";
    setDiscountType(nextType);
    setPercentage(nextEditor.scholarship ? String(nextType === "monto_fijo" ? nextEditor.scholarship.monto_fijo : nextEditor.scholarship.porcentaje) : "");
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    const numericDiscount = Number(percentage);
    if (normalizedName.length < 3) {
      setError("El nombre debe contener al menos 3 caracteres.");
      return;
    }
    if (!Number.isFinite(numericDiscount) || numericDiscount <= 0) {
      setError(discountType === "porcentaje" ? "El porcentaje debe ser mayor a 0." : "El monto fijo debe ser mayor a $0.00.");
      return;
    }
    if (discountType === "porcentaje" && numericDiscount > 100) {
      setError("El porcentaje debe ser menor o igual a 100.");
      return;
    }
    ...
    try {
      ...
      const values = {
        ...
        porcentaje: discountType === "porcentaje" ? Math.round(numericDiscount * 100) / 100 : 0,
        monto_fijo: discountType === "monto_fijo" ? Math.round(numericDiscount * 100) / 100 : 0,
        alcance: scope,
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        {!isLoading && scholarships.map((scholarship) => (
          <article key={scholarship.id} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h2 className="font-semibold text-slate-950">{scholarship.nombre}</h2><span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${scholarship.activa ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{scholarship.activa ? "Activa" : "Inactiva"}</span></div><p className="mt-3 text-3xl font-bold tabular-nums text-sky-700">{getScholarshipDiscountLabel(scholarship)}</p><p className="mt-1 text-xs font-medium text-slate-400">{scholarship.tipo_descuento === "monto_fijo" ? "Monto fijo" : "Porcentaje"}</p></div><GraduationCap className="h-6 w-6 text-sky-300" /></div>
