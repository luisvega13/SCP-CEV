...
import { getScholarshipDiscountLabel } from "@/lib/scholarships";
...
type ScholarshipAssignment = {
  ...
  tipo_descuento_aplicado: Beca["tipo_descuento"];
  ...
  becas: Pick<Beca, "id" | "nombre" | "porcentaje" | "tipo_descuento" | "monto_fijo" | "activa">;
};
...
export default function ScholarshipsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [assignmentsResult, studentsResult, scholarshipsResult, assignedResult] = await Promise.all([
        supabase
          .from("alumnos_becas")
          .select("id, alumno_id, beca_id, ciclo_escolar, observaciones, tipo_descuento_aplicado, porcentaje_aplicado, monto_fijo_aplicado, alcance_aplicado, vigencia_desde, fecha_asignacion, alumnos!inner(id, nombre, apellido_paterno, apellido_materno, matricula, nivel, grado, grupo), becas!inner(id, nombre, tipo_descuento, porcentaje, monto_fijo, activa)", { count: "exact" })
  ...
  async function handleAssign(event: FormEvent<HTMLFormElement>) {
    ...
    if (!assignmentPassword) {
      setError("Ingresa tu contraseña para autorizar la asignación.");
      return;
    }
    setIsSaving(true);
    setError("");
    try {
      const response = await fetch("/api/admin/scholarships/assign", {
        method: "POST",
      ...
      if (!response.ok || !result.assignment) {
        throw new Error(result.error || "No fue posible asignar la beca.");
      }
      invalidateAdminData("students:");
      invalidateAdminData("dashboard:");
      invalidateAdminData("reports:");
      setIsAssignModalOpen(false);
      const effectiveDate = new Intl.DateTimeFormat("es-MX", {
        day: "numeric",
        month: "long",
        year: "numeric",
        timeZone: "America/Mexico_City",
      }).format(new Date(`${result.assignment.vigencia_desde}T12:00:00-06:00`));
      setToast(`Beca asignada. El descuento inicia el ${effectiveDate}; los meses anteriores no cambiaron.`);
      await loadData();
    } catch (caughtError) {
  ...
  async function downloadScholarshipsCsv() {
    setIsDownloading(true);
    setError("");
    try {
      const response = await fetch(
        `/api/admin/exports/scholarship-students?cycle=${encodeURIComponent(cycle)}`,
      );
      if (!response.ok) {
      ...
      anchor.href = url;
      anchor.download = `alumnos-becados-${cycle}.csv`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
    } catch (caughtError) {
      setError(getErrorMessage(caughtError, "No fue posible descargar los alumnos becados."));
    } finally {
  ...
  async function removeAssignment() {
    ...
    try {
      ...
      setError(
        message.includes("ya existen pagos con el descuento aplicado")
          ? "No se puede retirar esta beca porque ya hay pagos cuyo importe fue calculado con ese descuento. Para conservar la integridad financiera, revisa y elimina primero esos pagos auditados desde el historial del alumno."
          : message,
      );
    } finally {
      setIsSaving(false);
    ...
    <section className="mx-auto max-w-7xl">
      <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <button type="button" onClick={() => void downloadScholarshipsCsv()} disabled={isLoading || isDownloading} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">{isDownloading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}Descargar becados CSV</button>
      ...
      <div className="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="min-w-[900px] w-full divide-y divide-slate-200">
            <thead className="bg-slate-50"><tr>{["Alumno", "CURP", "Nivel / Grado / Grupo", "Beca", "Descuento", "Asignación"].map((heading) => <th key={heading} className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">{heading}</th>)}<th className="px-5 py-3.5 text-right text-xs font-semibold uppercase tracking-wider text-slate-600">Acción</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              ...
              {!isLoading && assignments.map((assignment) => (
                <tr key={assignment.id} className="hover:bg-slate-50">
                  ...
                  <td className="whitespace-nowrap px-5 py-4 text-sm font-semibold tabular-nums text-slate-900"><span>{getScholarshipDiscountLabel(assignment)}</span><span className="ml-2 text-xs font-normal capitalize text-slate-400">{assignment.alcance_aplicado === "ambas" ? "Inscripción y mensualidad" : assignment.alcance_aplicado}</span></td>
                  <td className="whitespace-nowrap px-5 py-4 text-sm text-slate-500"><span className="block">Asignada: {new Intl.DateTimeFormat("es-MX", { dateStyle: "medium" }).format(new Date(assignment.fecha_asignacion))}</span><span className="mt-1 block text-xs font-medium text-sky-700">Descuento desde: {new Intl.DateTimeFormat("es-MX", { dateStyle: "medium", timeZone: "America/Mexico_City" }).format(new Date(`${assignment.vigencia_desde}T12:00:00-06:00`))}</span></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-[2px]">
          <section role="dialog" aria-modal="true" aria-labelledby="assign-scholarship-title" className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <form onSubmit={handleAssign} className="mt-6 space-y-5">
              ...
              <div className="rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm leading-6 text-sky-800">Si el ciclo ya inició, el descuento comenzará el próximo mes. Los cargos y pagos de meses anteriores conservarán sus importes originales.</div>
              <div><label htmlFor="scholarship-password" className="text-sm font-medium text-slate-700">Contraseña del administrador</label><input id="scholarship-password" required type="password" autoComplete="current-password" value={assignmentPassword} onChange={(event) => setAssignmentPassword(event.target.value)} className="mt-2 w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100" /><span className="mt-1 block text-xs text-slate-500">La confirmación quedará registrada en la auditoría de asignaciones.</span></div>
